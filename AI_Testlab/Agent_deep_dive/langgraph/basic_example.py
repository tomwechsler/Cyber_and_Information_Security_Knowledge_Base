...
def ensure_requirements() -> None:
    """Install the Python packages required to run the LangGraph example."""
    required_packages = [
        "langchain",
        "langchain-core",
        "langgraph",
    ...
    if not shutil.which("ollama"):
        raise RuntimeError(
            "Ollama was not found in PATH. Install Ollama from https://ollama.com "
            "and make sure the 'ollama' command works before running this script."
        )


def ensure_ollama_model(model_name: str) -> str:
    """Pull the configured Ollama model if it is not already available locally."""
    result = subprocess.run(["ollama", "list"], capture_output=True, text=True, check=False)
    if result.returncode != 0:
        raise RuntimeError(f"Unable to query local Ollama models: {result.stderr.strip() or result.stdout.strip()}")
...
# We'll use this local Ollama LLM to make decisions in our graph.
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", "qwen2.5:14b")
ensure_ollama_model(OLLAMA_MODEL)
model = ChatOllama(model=OLLAMA_MODEL, temperature=0.2, base_url="http://localhost:11434")
...
def analyze_url_tool(url: str) -> dict:
    ...
    if "https://zero.webappsecurity.com" in url:
...
initial_state_malicious = {"url": "https://zero.webappsecurity.com"} 
...
initial_state_benign = {"url": "https://zero.webappsecurity.com"} 
